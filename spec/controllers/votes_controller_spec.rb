...
RSpec.describe VotesController, type: :controller do
  describe 'POST #create', js: true do
    ...
    let!(:vote_params) { { vote_type: 'true', voteable_id: answer.id, voteable_type: 'Answer' } }

    scenario 'unauthenticated user cant vote' do
      expect { post :create, params: { vote:  vote_params }, format: :json }.to_not change(Vote.all, :count)
    end
    ...
    context 'authenticated user' do
      ...
      context 'existing vote' do
        ...
        scenario 'voting differently changes existing vote' do
          post :create, params: {
            vote: { vote_type: 'false', voteable_id: answer.id, voteable_type: 'Answer' }
            }, format: :json
          vote.reload
          expect(vote.vote_type).to eq false
        end
      end
    end
  end
